...
// --- LOS 3 BOLSILLOS (Estrategia Antcor) ---
const BUCKETS = [
...
// Formato de dinero: Maneja montos negativos (saldo rojo en personal) y formato sol peruano
...
export default function BilleteraApp() {
  ...
  const currentBalances = useMemo(() => {
    return transactions.reduce((acc, tx) => {
      ...
      if (tx.bucket === 'NEGOCIO_RAFAEL') acc.rafaelNegocio += amount;
      if (tx.bucket === 'NEGOCIO_CANO') acc.canoNegocio += amount;
      if (tx.bucket === 'PERSONAL_RAFAEL') acc.rafaelPersonal += amount;
      acc.total += amount;
      return acc;
    }, { rafaelNegocio: 0, canoNegocio: 0, rafaelPersonal: 0, total: 0 });
  }, [transactions]);

  // --- MÁQUINA DEL TIEMPO (Fix Timezone Lima UTC-5) ---
  ...
  // Búsqueda: Filtra primero y luego recorta si no hay término
  ...
  if (!user) return (
    <div className="flex h-screen items-center justify-center bg-[#002A8D] p-6 text-center">
      <div className="bg-white p-8 rounded-[2.5rem] w-full max-w-sm shadow-2xl">
        ...
        <button onClick={() => signInWithPopup(auth, new GoogleAuthProvider())} className="w-full bg-[#002A8D] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-blue-900/20">
           Acceder con Google
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex justify-center bg-gray-900 min-h-screen font-sans">
      <div className="w-full max-w-md bg-white shadow-2xl relative flex flex-col h-screen overflow-hidden">
          ...
          {activeTab === 'home' && (
            <div className="p-4 space-y-4">
              <div className="relative">
                ...
                <input 
                  type="text" 
                  placeholder="Buscar cliente, concepto o bolsillo..." 
                  value={searchTerm} 
                ...
                {filteredTransactions.map((tx) => (
                  <div key={tx.id} className={`bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between relative transition-all ${tx.status === 'anulado' ? 'opacity-20 grayscale' : ''}`}>
                    <div className="flex items-center gap-3 overflow-hidden">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shrink-0 ${BUCKETS.find(b => b.id === tx.bucket)?.color}`}>
                        {BUCKETS.find(b => b.id === tx.bucket)?.icon}
                      </div>
                    ...
                    <div className="text-right ml-2">
                      ...
                      <p className="text-[7px] font-black text-gray-400 uppercase tracking-widest">{tx.bucket.split('_')[0]}</p>
                    </div>
          ...
          {activeTab === 'audit' && (
            <div className="p-4 space-y-4">
              <div className="bg-white p-4 rounded-2xl border-2 border-[#002A8D]/10">
                <label className="text-[10px] font-black text-[#002A8D] uppercase mb-2 block tracking-widest">Corte de Auditoría (Lima):</label>
                 ...
                 {auditResult.list.map(tx => (
                   <div key={tx.id} className={`bg-white p-3 rounded-xl border flex justify-between items-center text-[10px] font-bold uppercase transition-opacity ${tx.status === 'anulado' ? 'opacity-30 line-through' : ''}`}>
                      <div className="flex gap-2 items-center overflow-hidden">
                         <span className={`w-2 h-2 rounded-full shrink-0 ${BUCKETS.find(b => b.id === tx.bucket)?.color}`}></span>
              ...
              <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                   <div className="bg-green-50 p-4 rounded-2xl flex justify-between items-center border border-green-100">
                      ...
                      <span className="text-xl font-black text-green-700">{formatMoney(transactions.filter(t => t.type === 'ingreso' && t.status !== 'anulado').reduce((a,c) => a+c.amount, 0))}</span>
                   </div>
                   <div className="bg-red-50 p-4 rounded-2xl flex justify-between items-center border border-red-100">
                      ...
                      <span className="text-xl font-black text-red-700">{formatMoney(transactions.filter(t => t.type === 'gasto' && t.status !== 'anulado').reduce((a,c) => a+c.amount, 0))}</span>
                   </div>
        ...
        {/* NAVEGACIÓN */}
        <div className="bg-white border-t border-gray-100 flex justify-around py-4 pb-8 absolute bottom-0 w-full z-20 shadow-[0_-10px_25px_rgba(0,0,0,0.05)]">
          ...
          <div className="absolute inset-0 z-50 flex items-end bg-[#00174f]/40 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white w-full rounded-t-[2.5rem] p-8 shadow-2xl animate-in slide-in-from-bottom-full overflow-y-auto max-h-[90%]">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                   ...
                   <div className="grid grid-cols-2 gap-2">
                      {['YAPE', 'PLIN', 'TRANSF.', 'EFECTIVO'].map(m => (
                        <button key={m} type="button" onClick={() => setFormData({...formData, method: m})} className={`py-3 rounded-xl border-2 text-[10px] font-black uppercase transition-all ${formData.method === m ? 'bg-gray-800 text-white border-gray-800 shadow-md' : 'bg-white text-gray-400 border-gray-100'}`}>{m}</button>
                      ))}
                   </div>
                </div>
                ...
                <button type="submit" className={`w-full py-5 rounded-[2rem] font-black text-white uppercase tracking-widest shadow-xl transition-all active:scale-95 italic ${formType === 'ingreso' ? 'bg-green-500 shadow-green-100' : 'bg-red-500 shadow-red-100'}`}>Guardar en Cloud</button>
              </form>
            </div>
          </div>
        )}
        ...
        {showProfile && (
          <div className="absolute inset-0 z-[60] flex items-center justify-center bg-[#00174f]/60 backdrop-blur-md p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl text-center relative animate-in zoom-in-95 duration-200">
              ...
              <p className="text-[10px] text-green-500 mb-6 uppercase font-black tracking-[0.2em] flex items-center justify-center gap-1">
                 <ShieldCheck size={12} /> Sincronización Activa
              </p>
              <div className="bg-gray-50 text-gray-400 font-mono text-[9px] py-4 px-4 rounded-2xl mb-8 break-all border border-gray-100 leading-tight italic">
                {user?.email}
              </div>
